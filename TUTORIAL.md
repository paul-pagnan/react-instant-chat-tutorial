...
- 1) key - the key is a React prop which tells the renderer the index of the current component in the loop. Remember that React's render engine only renders the changes by  a diff system. Thus, the key property is a way for React to determine which component it needs to re-render based on which element in the array has changed.
